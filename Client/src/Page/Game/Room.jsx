...
const Room = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      socketIO.off("RECEIVE_LIST_COORDINATES");
    };
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      socketIO.off("RECEIVE_CONTINUE_GAME");
    };
  }, []);
  ...
  const checkWinGame = (data) => {
    ...
    const isWin =
      utils.checkHorizontal(objTick, newTick) ||
      utils.checkVertical(objTick, newTick) ||
      utils.checkLeftCross(objTick, newTick) ||
      utils.checkRightCross(objTick, newTick);
    return isWin;
  };
  ...
  const isShowModalMessage = !!(
    checkWinGame(dataRoom) &&
    dataRoom.isWinGame &&
    player
  );
        ...
        <div style={{ width: "200px" }}>
          <div className="p-10">
            <div className="name-room">
              ...
              <div className="fz13 pointer" onClick={handleMoveRoom}>
                Rời phòng
              </div>
            </div>
            <PlayerCpn
            ...
            <div className="text-your-turn text-center m-10">
              {isTurnTick
                ? Constant.BOARD_GAME.message_your_turn
                : Constant.BOARD_GAME.message_your_competitor}
            </div>
            <div className="fz12 m-10">{Constant.BOARD_GAME.title_note}</div>
            {
              <ModalMessage
                isMessage={isTurnTick}
                dataRoom={dataRoom}
                player={player}
